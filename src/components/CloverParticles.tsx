...
const CloverParticles = () => {
  ...
  useEffect(() => {
    const newParticles = Array.from({ length: 25 }, (_, i) => ({
      ...
      opacity: Math.random() * 0.15 + 0.05, // Subtle opacity (0.05 - 0.2)
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden" aria-hidden="true">
      {particles.map((p) => (
        ...
        >
          <svg viewBox="0 0 512 512" fill="currentColor" className="w-full h-full drop-shadow-sm">
            <path d={CLOVER_PATH} />
          </svg>
        </div>
      ))}
